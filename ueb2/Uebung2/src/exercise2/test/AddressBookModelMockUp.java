/*
 * Example code used in exercises for lecture "Grundlagen des Software-Testens"
 * Created and given by Ina Schieferdecker and Edzard Hoefig
 * Freie Universitaet Berlin, SS 2012
 */
package exercise2.test;

import java.io.IOException;
import java.util.Collection;

import exercise2.addressbook.model.AddressBookModel;
import exercise2.addressbook.model.Entry;
import exercise2.addressbook.model.SizeLimitReachedException;

/**
 * Uebung 2 - Komponenten und Integrationstest
 * Mock-Up für das AddressBookModel
 * 
 * Bitte Nummer der Gruppe eintragen:
 * 10
 * 
 * Bitte Gruppenmitglieder eintragen:
 * @author René Perschon
 * @author Martin Schulze
 * @author Anselm Brachmann
 * @author Marcus Janz
 */
public class AddressBookModelMockUp implements AddressBookModel {

	boolean gotAnAdd=false;
	Entry lastEntryToAdd;
	
	
	public boolean getGotAnAdd(){
		boolean tmp=gotAnAdd;
		gotAnAdd=false;
		return tmp;
	}
	
	public Entry getLastEntryToAdd(){
		return lastEntryToAdd;
	}
	
	@Override
	public boolean addEntry(Entry entry) throws SizeLimitReachedException {
		if(entry.getFirstName().equals("SizeLimit"))
			throw new SizeLimitReachedException("For purpose triggered Exception from MockUp.");
		gotAnAdd=true;
		lastEntryToAdd=entry;
		return false;
	}

	@Override
	public Entry getEntry(String surName, String firstName) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public Collection<Entry> getEntries() {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public boolean deleteEntry(Entry entry) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public void erase() {
		// TODO Auto-generated method stub
		
	}

	@Override
	public void load() throws IOException {
		// TODO Auto-generated method stub
		
	}

	@Override
	public void save() throws IOException {
		// TODO Auto-generated method stub
		
	}

	// TODO: Mock-Up erstellen
	
}
